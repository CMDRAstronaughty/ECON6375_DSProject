...
```{r}
# Group the data by year and county 7a-7b
summary_tbl1 <- combined_df_Conventional %>% 
...
```{r}
# 7c
summary_tbl2 <- combined_df_Conventional %>% 
  filter(debt_to_income_ratio < 40) %>% 
  group_by(activity_year,county_code) %>% 
  summarise(cnt = n(),.groups = 'drop') %>%
  mutate(dti_percentage = round(cnt/sum(cnt),3)*100) %>% 
...
summary_tbl3 <- combined_df_Conventional %>%
  filter(business_or_commercial_purpose == 1) %>%
  group_by(activity_year,county_code) %>%
  summarise(cnt=n(),.groups = 'drop') %>%
  mutate(business_commerical_percent = round(cnt/sum(cnt),3)*100) %>%
...
summary_tbl4 <- combined_df_Conventional %>%
  filter(occupancy_type == 3) %>%
  group_by(activity_year,county_code) %>%
  summarise(cnt=n(),.groups = 'drop') %>%
  mutate(investment_property_percent = round(cnt/sum(cnt),3)*100) %>%
  as.data.frame()
```

```{r}
# Combine summary tables 2-4 into summary table (summary_tbl)
summary_tbl <- cbind(summary_tbl1,dti_percentage = summary_tbl2$dti_percentage)
summary_tbl <- cbind(summary_tbl,business_commerical_percentage = summary_tbl3$business_commerical_percent)
summary_tbl <- cbind(summary_tbl,investment_percentage = summary_tbl4$investment_property_percent)
```
...
```{r}
# Reshape Data
summary_tbl_reshape <- reshape(summary_tbl,idvar = "county_code",timevar = "activity_year",direction = 'wide')
```

##### b) Create a new variable with the percentage difference between 2021 and 2020

```{r}
# Here we'll take the absolute value as a function of the difference. So |2021 - 2020| for the percentages
